<x-app-layout>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-sky-900 text-center">
                    <p class="text-right md:text-xl font-semibold text-blue-900 mb-4">العناصر الموجودة في المخزن</p>
                    <div class="flex mb-2 justify-end">
                        <a href="{{ route(Auth::user()->user_type.'.storage.create') }}">
                            <x-primary-button>
                                {{ __('اضافة عنصر') }}
                            </x-primary-button>
                        </a>
                        ...
                        <table class="table-auto overflow-x-auto w-full border">
                            ...
                            @foreach($items as $item)
                                <tr class="hover:bg-slate-200 text-indigo-900">
                                    ...
                                    <td class="border px-2 md:px-4 py-2">
                                        <a href="#" class="xedit" data-name="item_in_stock" data-pk="{{ $item->id }}" data-title="Enter item in stock" id="item_in_stock" data-type="number" data-url="{{url(Auth::user()->user_type.'/storage/update')}}">{{ $item->item_in_stock }}</a>
                                    <td class="border px-2 md:px-4 py-2 overflow-hidden overflow-ellipsis whitespace-nowrap max-w-xs sm:max-w-full break-all">
                                        <a href="#" class="xedit" data-name="item_price" data-type="number" data-pk="{{ $item->id }}" data-title="Enter item price" data-url="{{url(Auth::user()->user_type.'/storage/update')}}">{{ $item->item_price }}</a>$
                                    </td>
                                    <td class="border px-2 md:px-4 py-2">
                                        <a href="#" class="xedit" data-name="item_name" data-type="text" data-pk="{{ $item->id }}" data-title="Enter item name" data-url="{{url(Auth::user()->user_type.'/storage/update')}}">{{ $item->item_name }}</a>
                                    </td>
                                </tr>
                            @endforeach
                            </tbody>
    ...
    <x-modal name="confirm-user-deletion"
             ...
             focusable>
        <form method="post" action="{{ url(Auth::user()->user_type.'/storage/delete') }}" class="p-6">
            @csrf
            @method('delete')
